...
// No need for __dirname or __filename as we use process.cwd() for stability across ESM/CJS and bundling.
...
async function startServer() {
  ...
  app.post('/api/ai/ask', async (req, res) => {
    try {
      ...
      const apiKey = process.env.AI_CHAT_API || process.env.VITE_CUSTOM_GEMINI_API_KEY || process.env.GEMINI_API_KEY;
      if (!apiKey) {
      ...
      res.status(500).json({ success: false, text: `Oops! Something went wrong: ${error?.message || 'Unknown error'}` });
    }
  });
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (fs.existsSync(distPath)) {
      ...
      // Explicit sitemap.xml route to ensure correct Content-Type
