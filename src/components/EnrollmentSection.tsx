...
import { Plus, Link as LinkIcon, AlertCircle, CheckCircle, UserPlus, IndianRupee, X, Settings, Edit, Trash2, Check, FileText, Lock, Download, FileUp, ChevronDown, ChevronRight, Shield } from 'lucide-react';
...
export default function EnrollmentSection({ branding }: { branding?: any }) {
  ...
  const handleLogPayment = async () => {
    ...
    try {
      ...
      nextMonth.setDate(branding?.advancedPaymentDiscountDay || 5); // Use configured day or default to 5th
      ...
      // Update Academic Ledgers
      await pricingService.verifyPaymentAndApplyToLedger(selectedStudent.id, amount, 'custom_admin_offline_' + Date.now());
  ...
  useEffect(() => {
    let unsubEnrollments = firestoreService.listenToCollection('enrollments', setEnrollments);
  ...
  const renderBatch = (gradeTitle: string, gradeLevel: string) => {
    ...
    const visibleBatchStudents = isExpanded ? batchStudents : [];
    const hiddenCount = batchStudents.length;
    if (gradeLevel !== 'XII' && gradeLevel !== 'XI' && gradeLevel !== 'X') return null;
    
    // Find matching batch config
    const batchConfig = batches.find(b => b.systemGrade === gradeLevel || b.name === `CLASS ${gradeLevel}` || b.name === `Class ${gradeLevel}` || b.name === `Batch ${gradeLevel}`);
    ...
    return (
      <div className="mb-6 bg-white/5 dark:bg-[#111928]/40 rounded-2xl border border-[var(--border-color)] overflow-hidden">
            ...
            {batchConfig.waitlistMessage && (
              <p
                ...
                style={{
                  ...
                  textAlign: batchConfig.waitlistTextAlign || 'left',
                  fontWeight: 700,
                  whiteSpace: 'pre-wrap'
                }}
              >
        ...
        <div 
          className="bg-[var(--primary)]/10 p-3 border-b border-[var(--border-color)] flex justify-between items-center cursor-pointer hover:bg-[var(--primary)]/15 transition-colors"
          onClick={() => {
            const willExpand = !expandedBatchStudents[gradeLevel];
            analyticsService.logEvent({
              event: 'click',
              section: 'enrollment_list',
              itemName: gradeLevel,
              itemId: willExpand ? 'expand' : 'collapse'
            });
            setExpandedBatchStudents((prev) => ({ ...prev, [gradeLevel]: !prev[gradeLevel] }));
          }}
        >
          <div className="flex items-center gap-2">
            {isExpanded ? <ChevronDown size={18} className="text-[var(--primary)]" /> : <ChevronRight size={18} className="text-[var(--primary)]" />}
            ...
            <>
              ...
              {batchStudents.length > 0 && (
                ...
                >
                  ...
                  {isExpanded ? 'Hide Student List' : `View Student List (${batchStudents.length} Students)`}
                </button>
              )}
            </>
          )}
              ...
              {isEditingAdmin ? (
                ...
                <>
                  ...
                  <div className="bg-white/5 border border-[var(--border-color)] p-3 rounded-xl flex items-center justify-between">
                    <div>
                      <div className="text-xs opacity-70 mb-1 flex items-center gap-1"><Shield size={14} /> Verification Override</div>
                      <div className={`font-bold ${selectedStudent.manualVerificationOverride ? 'text-green-500' : 'text-gray-400'}`}>
                        {selectedStudent.manualVerificationOverride ? 'Verified (Manual Override)' : 'Delegated to Ledger'}
                      </div>
                      <div className="text-[10px] opacity-70 max-w-[150px] mt-1">This overrides ledger checks and forces access.</div>
                    </div>
                    <button 
                      onClick={() => {
                        const nextValue = !(selectedStudent as any).manualVerificationOverride;
                        handleAdminUpdate(selectedStudent.id, { manualVerificationOverride: nextValue });
                        setSelectedStudent({...selectedStudent, manualVerificationOverride: nextValue} as unknown as Enrollment);
                      }}
                      className={`px-3 py-1.5 rounded-lg text-xs font-bold ${(selectedStudent as any).manualVerificationOverride ? 'bg-red-500/20 text-red-500 hover:bg-red-500/30' : 'bg-green-500/20 text-green-500 hover:bg-green-500/30'} transition-all shrink-0`}
                    >
                      {(selectedStudent as any).manualVerificationOverride ? 'Revoke Override' : 'Force Verify'}
                    </button>
                  </div>
                  ...
                  <div className="bg-white/5 border border-[var(--border-color)] p-3 rounded-xl flex items-center justify-between">
                    ...
                    >
                      <IndianRupee size={14} /> Log Custom Payment
                    </button>
                  </div>
            ...
            {showPaymentModal && (
              <div className="absolute inset-0 z-50 bg-gray-900 rounded-[var(--radius)] p-6 flex flex-col justify-center animate-in fade-in zoom-in duration-200">
                <h3 className="text-xl font-bold mb-2 flex items-center gap-2 text-[var(--success)]">
                  <IndianRupee /> Log Custom Payment
                </h3>
