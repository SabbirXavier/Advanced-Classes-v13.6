        ...
        <div className="w-full h-full p-2 md:p-8 flex items-center justify-center">
          {(material.type === 'pdf' || (material.url && material.url.includes('.pdf'))) ? (
            <div className="w-full h-full max-w-6xl rounded-2xl overflow-hidden shadow-2xl relative bg-white/5 border border-white/10">
              ...
              <div className="absolute inset-0 pointer-events-none z-50 overflow-hidden flex flex-wrap justify-center items-center opacity-[0.03] sm:opacity-[0.04]">
                 {Array.from({ length: 150 }).map((_, i) => (
                   <div 
                     key={i} 
                     className="transform -rotate-45 text-black font-black text-xl whitespace-nowrap p-4 select-none" 
                     style={{ WebkitTextStroke: '1px white' }}
                   >
                     {user?.email || user?.uid} {user?.phoneNumber || ''}
                   </div>
                 ))}
              </div>
            </div>
